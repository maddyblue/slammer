/*==========================================================================*/
/*                                                                          */
/* Decoupled and Couplded Block Sliding Analysis Program                    */
/*                                                                          */
/* Created:    5/04/2004                                                    */
/* Modified :  5/21/2004                                                    */
/* Description : Programa for calculationg sliding displacement with        */
...
void decoupled(char filename1[], char filename2[], char filename3[], double &uwgt, double &height, double &vs, double &damp, double &dt, int &npts, double &scal, double &g, int &nhead, int &npl, double &vr, int &nmu, double disp[], double mu[],int dv1, int &dv2, int &dv3, double vs1);
...
void d_sliding(int qq, int j, double dt, int npts, int &slide, double mu[], double g, double avgacc[], double sdot[], double s[]);
...
void coupled(char filename1[], char filename2[], char filename3[], double &uwgt, double &height, double &vs, double &damp, double &dt, int &npts, double &scal, double &g, int &nhead, int &npl, double &vr, int &nmu, double disp[], double mu[],int dv1, int &dv2, int dv3, double vs1);
...
void output1(char filename2[], char descrip[], double rho, double height, double vs, double vr, double damp, int nmu, double mu[], double disp[], double dampf, int dv1);
...
void avg_acc(char filename2[], double avgacc[], int npts, double &mmax);
...
int main()
{
	...
	if(dv1==0)
	{
		decoupled(filename1, filename2, filename3, uwgt, height, vs, damp, dt, npts, scal, g, nhead, npl, vr, nmu, disp, mu,dv1, dv2, dv3, vs1);
	}
	else if (dv1==1)
	{
		coupled(filename1, filename2, filename3, uwgt, height, vs, damp, dt, npts, scal, g, nhead, npl, vr, nmu, disp, mu,dv1, dv2, dv3, vs1);
...
double Mtot, M, L, omega, avgacc[10000]={0.0}, deltacc=0.0;
...
double mx=0.0, mx1=0.0, mmax=0.0,gameff1=0.0;
double gamref;
...
///////////////////////////////////////////
	...
	output1(filename2, descrip, rho, height, vs, vr, damp, nmu, mu, disp, dampf, dv1);
...
// For Linear Elastic
	...
	avg_acc(filename2, avgacc, npts, mmax);
...
for(j=1;j<=npts;j++)
{
    ...
    d_sliding(qq, j, dt, npts, slide, mu, g, avgacc, sdot, s);
...
void d_eq(char filename2[], double n, double o, int npts, double g, double scal, double u1, double udot1, double udotdot1, double u[], double udot[], double udotdot[], double acc1, double acc2, double ain[], double s[], int slide, double omega, double &damp, double rho, double height, double dt, double avgacc[], double &vs, double gameff1, double gamref, double dampf, double pi, double uwgt, double vr, int dv2, double vs1)
{
	...
	fout.setf(ios_base::right, ios_base::floatfield);
	fout.setf(ios::fixed);
	//fout<<setw(10)<<setprecision(5)<<s[j-1]<<endl;
...
void output2(char filename2[], double s[], int j, double dt)
{
	...
	fout.setf(ios_base::right, ios_base::floatfield);
	fout.setf(ios::fixed);
	//fout<<setw(10)<<setprecision(5)<<s[j-1]<<setw(10)<<setprecision(5)<<avgacc[j-1]/32.2<<setw(10)<<setprecision(5)<<ain[j-1]<<endl;

	if(j==1)
	{
	fout<<setw(10)<<"TIME"<<setw(30)<<"Sliding Displ."<<endl;
	}
	fout<<setw(10)<<setprecision(4)<<j*dt<<setw(25)<<setprecision(5)<<fabs(s[j-1])<<endl;
	//fout<<setw(10)<<setprecision(5)<<"vs= "<<vs<<setw(10)<<setprecision(5)<<"damp= "<<damp<<endl;
}
...
double mx=0.0, mx1=0.0, mmax=0.0,gameff1=0.0, gamref,n, o;
double s[5000]={0.0},u[5000]={0.0};
